class Igrac(object):
    ...
    def pozicija_string(self, pozicija):
        pozicije = ("A", "B", "C", "D", "E", "F", "G", "H")
        return pozicije[pozicija[1]] + str(pozicija[0] + 1)
    
    def moguci_potezi_string(self, tabla):
        self.izracunaj_moguce_poteze(tabla)
        if len(self.moguci_potezi) == 0:
            tabla.string_za_ispis = "Ne postoji potez koji možete odigrati, vaš red se preskače!"
        else:
            tabla.string_za_ispis = "Mogući potezi su: "
            for potez in self.moguci_potezi:
                tabla.string_za_ispis += self.pozicija_string(potez) + ", "
            tabla.string_za_ispis = tabla.string_za_ispis[:-2] + "\n"
    
    def string_u_potez(self, string):
        return (int(string[1]) - 1, ord(string[0]) - 65)
    
    def odigraj_potez(self, tabla, string, protivnik):
        potez = self.string_u_potez(string)
        self.izracunaj_moguce_poteze(tabla)
